...
import { useDownloadManager } from '../Hook/use-download-manager'
...
const Index: React.FC = () => {
  ...
  const { downloadList, downloadPath } = useSelector((state: RootState) => state.counter)
  const { hasActiveDownloads, downloads, history, addDownload, removeDownload, clearHistory } = useDownloadManager()
  const downloadingRef = useRef(new Set<string>()) //防止重复下载
  // 处理滚动动画
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    const pendingList = downloadList.filter((item) => !downloadingRef.current.has(item.id))
    for (const item of pendingList) {
      downloadingRef.current.add(item.id) // 标记为正在下载
      // 获取MP3 下载地址
      createSongInfo(item).then((res) => {
        const mp3Url = res?.data.mp3_url
        if (!mp3Url) {
          console.log('获取 MP3_url 失败')
          downloadingRef.current.delete(item.id)
          return
        }
        console.log(item, 'ite')
        const filename = `${item.music_title}-${item.artist}.mp3`
      })
    }*/
    console.log('监听到下载的歌曲', downloadList)
  }, [downloadList])
  // 点击处理
  ...
  const getOverallProgress = (): number => {
    ...
    const totalProgress = downloads.reduce((sum, item) => sum + item.progress, 0)
        ...
        <>
          ...
          <div className="absolute top-12 right-0 w-96 bg-gray-800 border border-gray-600 rounded-lg shadow-xl z-50 overflow-hidden animate-in slide-in-from-top duration-200">
            ...
            <div className="flex items-center justify-between p-4 border-b border-gray-600">
              ...
              <button onClick={() => setShowPanel(false)} className="text-gray-400 hover:text-white transition-colors">
                <X size={18} />
              </button>
            </div>
                            ...
                            <div className="flex items-center justify-between relative z-10">
                              <div className="flex-1 min-w-0 flex items-center gap-2">
                                <div className="flex-1 min-w-0">
                                  ...
                                  <span className="text-gray-400 text-xs ml-1">- {item.artist}</span>
                                </div>
                              </div>
                              <div className="flex items-center gap-1.5">
                                <span
                                  className={`text-xs font-bold ${
                                    item.status === "downloading" ? "text-green-400" : "text-blue-400"
                                  }`}
                                >
