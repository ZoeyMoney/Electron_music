...
const MusicList: React.FC<MusicListProps> = ({ title, list, showViewMore = true, loading, onClick }) => {
      ...
      <div className="grid grid-cols-2">
        ...
        {list.length <= 9 || !showViewMore ? null : (
          <h2 className={'pb-5 text-end text-[14px]'}>
            <span className={'cursor-pointer text-[#808080] hover:text-white transition-all'} onClick={viewMoreClick}>
              显示更多
            </span>
          </h2>
        )}
              ...
              index <= 9 || !showViewMore ? (
                <Card key={index} isPressable shadow="sm" onPress={
                  ...
                  <CardBody className="overflow-visible p-0 relative buttonsHover">
                    <img src={item.pic} alt="" style={{ borderRadius: '5px', height: '140px' }} />
                  </CardBody>
                  <CardFooter className="text-small justify-between py-[13px] px-[3px]">
                    <b className={'whitespace-nowrap overflow-hidden w-[140px] text-[12px] mx-auto'}>
                      {item.name}
                    </b>
                  </CardFooter>
                </Card>
